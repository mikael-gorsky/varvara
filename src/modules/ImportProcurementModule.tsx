...
const ImportProcurementModule: React.FC = () => {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const rows = await parseProcurementFile(selectedFile);

      setProgress({
        status: 'parsing',
        message: `Parsed ${rows.length} products with sales history...`
      });
      ...
      if (result.success) {
        setProgress({
          ...
          salesRecords: result.stats?.salesRecords,
          suppliers: result.stats?.suppliers,
          months: result.stats?.months,
    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-gray-700 pb-4">
        ...
        <p className="text-sm text-gray-400 mt-1">
          Upload procurement planning Excel file (File 3: 13.02.26_шред ОК.xlsx)
        </p>
      </div>
      ...
      <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
        <div className="space-y-4">
          ...
          >
            ...
            {progress.status !== 'parsing' && progress.status !== 'uploading' && 'Import Procurement Data'}
          </button>
        </div>
      </div>
              ...
              {progress.status === 'complete' && (
                <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className="bg-gray-800/50 rounded px-3 py-2">
                    ...
                    <div className="text-lg font-semibold text-gray-100">
                      {progress.salesRecords?.toLocaleString() || '—'}
                    </div>
                  </div>
                  <div className="bg-gray-800/50 rounded px-3 py-2">
                    <div className="text-xs text-gray-400">Suppliers</div>
                    <div className="text-lg font-semibold text-gray-100">
                      {progress.suppliers?.toLocaleString() || '—'}
                    </div>
                  </div>
                  <div className="bg-gray-800/50 rounded px-3 py-2">
                    <div className="text-xs text-gray-400">Months</div>
                    <div className="text-lg font-semibold text-gray-100">
                      {progress.months?.toLocaleString() || '—'}
                    </div>
                  </div>
                </div>
              )}
      ...
      <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
        ...
        <ul className="text-xs text-gray-400 space-y-1 list-disc list-inside">
          <li>Excel file (.xlsx or .xls) with procurement planning data</li>
          <li>Required columns: Номенклатура (Product Name)</li>
          <li>Optional columns: Поставщик (Supplier), Цена (Price USD), Объем (Volume CBM)</li>
          <li>Monthly sales columns: янв.25, фев.25, etc. or 01.2025, 02.2025, etc.</li>
          <li>Header row should contain column names (auto-detected)</li>
          <li>Imports sales history, supplier data, and product dimensions</li>
        </ul>
      </div>
    </div>
  );
...
export default ImportProcurementModule;
